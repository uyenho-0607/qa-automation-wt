...
from data_config.postman.request import create_mt4_market_order
    ...
    def test_tc01(self, chromeDriver, request):
        ...
        try:
            ...
            with allure.step("Search symbol"):
                input_symbol(driver=main_driver, server=Server.MT4, client_name="Lirunex", desired_symbol_name="DASHUSD.std")
                
                # create_market_order(driver=main_driver)

            with allure.step("Bulk Close Orders"):
                clear_orderIDs_csv(filename="MT4_Bulk.csv")
                open_position_df = button_bulk_operation(driver=main_driver, filename="MT4_Bulk.csv", bulk_type="close", options_dropdown="all", section_name="Open Position", tab_order_type="open-positions")
