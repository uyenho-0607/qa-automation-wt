...
run_test_iteration() {
    ...
    (
        pytest tests/$PLATFORM/chart -m critical --user="569203282" --account=live --password='Autotest@12345' --client=transactCloud --alluredir="$allure_transactCloud"
    ...
    for dir in "$allure_lirunex_mt5" "$allure_lirunex_mt4" "$allure_transactCloud"; do
        if [ -d "$dir" ]; then
            echo "Merging: $dir"
            cp -r "$dir"/* "$merge_dir"/
        fi
