...
from common.mobileweb.module_trade.utils import toggle_radioButton_OCT, trade_stop_order, modify_stop_order, get_trade_snackbar_banner, extract_order_info
from data_config.utils import compare_dataframes, process_and_print_data, clear_orderIDs_csv, append_orderIDs_to_csv


@allure.epic("MT4 Mobile TS_aG - Stop OCT")

# Member Portal 
class TC_MT4_aG01():

    @allure.title("TC_MT4_aG01")
    ...
    def test_TC01(self, chromeDriver):
        ...
        try:
            ...
            with allure.step("Place Stop Order"):
                trade_stop_order(driver=main_driver, trade_type="trade", option="buy", set_stopLoss=False, set_takeProfit=False, expiryType="good-till-cancelled", pre_trade=True)
            ...
            with allure.step("Modify Stop Order"):
                modify_stop_order(driver=main_driver, trade_type="edit", row_number=1, sl_type="price", set_takeProfit=False, expiryType="good-till-cancelled")
            ...
            with allure.step("Retrieve the Updated Order Panel data"):
                ...
                clear_orderIDs_csv(filename="MT4_Mobile_Stop_OCT.csv")
                append_orderIDs_to_csv(order_ids=updated_orderID, filename="MT4_Mobile_Stop_OCT.csv")
