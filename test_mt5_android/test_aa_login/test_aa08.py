    ...
    def test_tc08(self, android_driver):
        ...
        try:
            with allure.step("Check the 'Rmb me' checkbox > Login to Web Trader Membersite > logout"):
                username, password = toggle_remember_me_checkbox(driver=main_driver, server=Server.MT5)
                
            with allure.step("Input incorrect credential"):
                login_wt(driver=main_driver, server=Server.MT5, testcase_id="TC02", expectation=AlertType.FAILURE) 
