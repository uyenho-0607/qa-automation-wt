...
from enums.main import Server, ButtonModuleType, OrderExecutionType, TradeConstants, TradeDirectionOption, ExpiryType, OrderPanel, SectionName, CSVFileNameManager, CSVFileNameManager
...
@allure.parent_suite("MT5 Membersite - Android - Trade - Stop Order")

@allure.epic("MT5 Andorid ts_af - Stop")

# Member Portal
class TC_aF02():

    @allure.title("TC_aF02")
    ...
    def test_tc02(self, android_driver):
        ...
        try:
            ...
            with allure.step("Place Stop Order"):
                trade_pending_order(driver=main_driver, order_type=OrderExecutionType.STOP, option=TradeDirectionOption.BUY, expiry_type=ExpiryType.GOOD_TILL_DAY)
            ...
            with allure.step("Modify Stop Order"):
                # modify_stop_order(driver=main_driver, expiryType="specified-date-and-time", expiry_date="19", target_month=datetime.strptime("April 2025", "%b %Y"), hour_option="11", min_option="35")
                modify_pending_order(driver=main_driver, order_type=OrderExecutionType.STOP, expiry_type=ExpiryType.SPECIFIED_DATE_AND_TIME, expiry_date="19", target_month=parse("April 2025"), hour_option="11", min_option="35")
            ...
            with allure.step("Verify if it is the same order_ids"):
                if original_order_id == updated_order_id:
                    append_order_ids_to_csv(order_ids=updated_order_id, filename=CSVFileNameManager.MT5_DESKTOP_PENDING_ORDER)
