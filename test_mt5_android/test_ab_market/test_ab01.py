...
from common.mobileapp.module_notification.noti_order import process_order_notifications
from data_config.data_comparison import compare_dataframes, process_and_print_data
    ...
    def test_TC01(self, android_driver):
        ...
        try:
            ...
            with allure.step("Place Market Order"):
                trade_market_order(driver=main_driver, option=TradeDirectionOption.BUY, trade_constants=TradeConstants.SET_FILL_POLICY)
            ...
            with allure.step("Click on the Trade Confirmation button to place the order"):
                edit_confirmation_df, confirmation_order_id = trade_orders_confirmation_details(driver=main_driver, trade_type=ButtonModuleType.EDIT)
            ...
            with allure.step("Verify if it is the same order_ids"):
                ...
                if confirmation_order_id != order_no:
                    assert False, f"Place orderID - {original_order_id} and retrieving orderID - {confirmation_order_id} not matched"
            ...
            with allure.step("Print Modify Order Table Result"):
                process_and_print_data(updated_order_df, edit_confirmation_df, edit_snackbar_banner_df, updated_order_df, noti_msg_df, noti_order_df)
