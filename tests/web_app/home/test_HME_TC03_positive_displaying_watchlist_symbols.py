...
def test(web_app, tab, setup_test):
    ...
    web_app.trade_page.watch_list.select_tab(tab)

    logger.info(f"Verify current tab displays {len(exp_symbols)} symbols")
    web_app.trade_page.watch_list.verify_symbols_list(symbols=exp_symbols)

    logger.info("Step 2: Get random symbol to select")
    select_symbol = web_app.trade_page.watch_list.get_current_symbols(random_symbol=True)

    logger.info(f"Step 3: Select {select_symbol!r}")
    web_app.trade_page.watch_list.select_symbol(select_symbol)
