...
def test_missing_all_required_fields(web_app):
    ...
    web_app.login_page.demo_account_modal.fill_demo_account_creation_form(account_info)
    web_app.login_page.demo_account_modal.tap_agree_and_continue()

    logger.info(f"Verify validation error messages for fields: {validation_fields!r}")
    web_app.login_page.demo_account_modal.verify_field_validation(fields=validation_fields, check_agreement=False)


def test_invalid_email_and_phone_number_format(web_app):
    ...
    logger.info(f"Verify validation error messages for fields: {validation_fields!r}")
    web_app.login_page.demo_account_modal.verify_field_validation(fields=validation_fields, validation_type="invalid")


@pytest.mark.parametrize("missing_field", random.choices(list(DemoAccountModal.validation_fields.keys())))
def test_single_missing_field(web_app, missing_field):
    ...
    logger.info(f"Step 2: Submit demo account creation form without input field: {missing_field!r}")
    time.sleep(1)
    web_app.login_page.demo_account_modal.fill_demo_account_creation_form(account_info)
    web_app.login_page.demo_account_modal.tap_agree_and_continue()

    logger.info(f"Verify validation message for missing {missing_field}")
    web_app.login_page.demo_account_modal.verify_field_validation(fields=[missing_field])