...
def test(web_app, stop_obj, edit_field, sl_type, tp_type, order_data, cancel_all):
    ...
    trade_object.order_id = web_app.trade_page.asset_tab.get_last_order_id(tab, True)

    logger.info(f"Verify order placed successfully, order_id: {trade_object.order_id!r}")
    web_app.trade_page.asset_tab.verify_item_data(trade_object, tab, False)

    logger.info(f"Step 4: Update {tab.title()} item with {edit_field!r} ({sl_type}, {tp_type})")
    web_app.trade_page.asset_tab.modify_order(trade_object, sl_type=sl_type, tp_type=tp_type, confirm=False, oct_mode=True)
