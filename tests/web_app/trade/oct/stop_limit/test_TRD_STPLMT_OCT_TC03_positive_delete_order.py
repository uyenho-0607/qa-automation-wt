...
def test(web_app, order_data, stop_limit_obj, get_asset_tab_amount, cancel_all):
    ...
    trade_object.order_id = web_app.trade_page.asset_tab.get_last_order_id(tab, wait=True)

    logger.info(f"Verify order placed successfully, order_id: {trade_object.order_id}")
    web_app.trade_page.asset_tab.verify_item_data(trade_object, wait=False)

    logger.info("Step 4: Delete order")
    web_app.trade_page.asset_tab.delete_order(trade_object, confirm=False)
