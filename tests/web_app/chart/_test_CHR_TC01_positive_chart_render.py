...
from uyn.statistic import stats

pytestmark = [pytest.mark.critical]

# Expected render time (seconds)
EXP_TIME = RuntimeConfig.charttime
SYMBOLS = {Client.LIRUNEX: "AAPL", Client.TRANSACT_CLOUD: "BTC.USD"}
...
def test_default(web_app):
    ...
    stats.add_render_time(RuntimeConfig.user, SYMBOLS[RuntimeConfig.client], "default", elapsed)
    ...
    for idx, timeframe in enumerate(timeframe_list):
        ...
        stats.add_render_time(RuntimeConfig.user, SYMBOLS[RuntimeConfig.client], timeframe, elapsed)
