...
def test(ios, tab, setup_test):
    ...
    ios.home_screen.watch_list.select_tab(tab)

    logger.info(f"Verify current tab displays {len(exp_symbols)} symbols: {', '.join(exp_symbols)}")
    ios.home_screen.watch_list.verify_symbols_list(exp_symbols)

    logger.info("Step 3: Get random displaying symbol")
    select_symbol = ios.home_screen.watch_list.get_random_symbol()

    logger.info(f"Step 4: Select {select_symbol!r}")
    ios.home_screen.watch_list.select_symbol(select_symbol)
    ...
    def _handler(tab):
        ...
        if tab != WatchListTab.FAVOURITES:
            ...
            if symbols and len(symbols) > 10:
                ...
                filtered_symbol = [sym for sym in symbols if
                                   not any(sym.startswith(letter) for letter in ['U', 'V', 'W', 'X', 'Y', 'Z'])]
                if filtered_symbol:
        ...
        logger.info(
            f">> Setup Summary: List symbols should be displayed in tab: {tab.value!r} - {', '.join(symbols)!r}",
            setup=True)
