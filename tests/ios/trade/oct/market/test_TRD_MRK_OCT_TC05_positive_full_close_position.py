...
def test(ios, symbol, get_asset_tab_amount, market_obj):
    trade_object = market_obj()
    tab_amount = get_asset_tab_amount(trade_object.order_type)

    logger.info(f"Step 1: Place order with: {format_display_dict(trade_object)} via OCT form (tab:{tab_amount})")
    ...
    ios.trade_screen.asset_tab.full_close_position(trade_object.order_id, False)
    trade_object.current_price = ios.trade_screen.asset_tab.get_current_price(
        trade_object.trade_type, trade_object.order_type, oct_mode=True)
