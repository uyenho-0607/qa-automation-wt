...
def test(ios, setup_teardown, symbol):
    acc_balance, acc_info, order_ids = setup_teardown

    logger.info("Step 1: Navigate to Asset screen")
    ios.home_screen.navigate_to(Features.ASSETS)

    logger.info("Verify account info (against API data)")
    ios.assets_screen.verify_account_details(acc_info)

    logger.info("Verify account balance summary (against API data)")
    ios.assets_screen.verify_account_balance_summary(acc_balance)

    logger.info("Step 2: Navigate to Home screen")
    ios.assets_screen.navigate_to(Features.HOME)
    ...
    for i, _id in enumerate(order_ids):
        profit_loss += ios.trade_screen.asset_tab.get_profit_loss()[0]

        ios.trade_screen.asset_tab.full_close_position(order_id=_id)

        logger.info(f"Verify order closed successfully (id: {_id!r})")
        ios.trade_screen.asset_tab.verify_items_displayed(AssetTabs.OPEN_POSITION, _id, is_display=False)

    logger.info("Step 5: Navigate back to Asset screen")
    ios.trade_screen.navigate_to(Features.ASSETS)
    ...
    ios.assets_screen.verify_account_balance_summary(acc_balance, tolerance_percent=1,
                                                     tolerance_fields=[AccInfo.BALANCE, AccInfo.REALISED_PROFIT_LOSS])
...
def setup_teardown(ios, symbol):
    ...
    logger.info(f">> Setup Summary: Placed order ID: {', '.join(str(item) for item in order_ids)}", setup=True)
