...
def test(ios):
    ...
    ios.login_screen.login(password=random_password())

    logger.info(f"Verify login error message is displayed")
    ios.login_screen.verify_alert_error_message()

    logger.info("Step 2: Login with invalid userid and valid password")
    ios.login_screen.login(userid=random_userid())

    logger.info(f"Verify login error message is displayed")
    ios.login_screen.verify_alert_error_message()

    logger.info("Step 3: Login with both invalid userid and password")
    ios.login_screen.login(random_userid(), random_password())
