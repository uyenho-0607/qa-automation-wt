...
def test_missing_all_required_fields(ios):
    ...
    ios.login_screen.demo_account_modal.tap_agree_and_continue()

    logger.info(f"Verify validation error messages for fields: {validation_fields!r}")
    ios.login_screen.demo_account_modal.verify_field_validation(fields=validation_fields, check_agreement=False)


def test_invalid_email_and_phone_number_format(ios):
    ...
    logger.info(f"Verify validation error messages for fields: {validation_fields!r}")
    ios.login_screen.demo_account_modal.verify_field_validation(fields=validation_fields, validation_type="invalid")


@pytest.mark.parametrize("missing_field", random.choices(list(DemoAccountModal.validation_fields.keys())))
def test_single_missing_field(ios, missing_field):
    ...
    ios.login_screen.demo_account_modal.verify_field_validation(fields=[missing_field])
