...
def test(android, market_obj, edit_field, sl_type, tp_type, order_data, cancel_all):
    trade_object = market_obj()

    logger.info(f"Step 1: Place order: {format_display_dict(trade_object)} without SL and TP")
    order_data(trade_object, None, None)

    trade_object.order_id = android.trade_screen.asset_tab.get_last_order_id(AssetTabs.OPEN_POSITION, True)

    logger.info(f"Verify order placed successfully, order_id: {trade_object.order_id!r}")
    android.trade_screen.asset_tab.verify_item_data(trade_object, AssetTabs.OPEN_POSITION)
