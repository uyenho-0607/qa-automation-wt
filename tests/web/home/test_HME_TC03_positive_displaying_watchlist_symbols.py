...
def test(web, tab, setup_test):
    ...
    logger.info(f"Step 1: Select tab: {tab.value!r}")
    web.home_page.watch_list.select_tab(tab)

    logger.info(f"Verify current tab displays {len(exp_symbols)} symbols: {', '.join(exp_symbols)} (result from API)")
    web.trade_page.watch_list.verify_symbols_list(tab, exp_symbols)

    logger.info("Step 2: Get random symbol from current display symbols")
    select_symbol = web.home_page.watch_list.get_random_symbol()

    logger.info(f"Step 3: Select symbol: {select_symbol!r} (result from step 2)")
    web.home_page.watch_list.select_symbol(select_symbol)

    logger.info(f"Verify symbol {select_symbol!r} selected in watchlist")
    web.home_page.watch_list.verify_symbol_selected(select_symbol)

    logger.info(f"Verify symbol {select_symbol!r} displayed in chart")
