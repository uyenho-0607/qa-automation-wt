...
def test(web, stop_limit_obj, get_asset_tab_amount, symbol, close_confirm_modal, update_trade_obj):
    step_idx = 2
...
def update_trade_obj(symbol):
    def _handler(order_data):
        ...
        if order_data.get("order_type") in [f"{item.upper()} {OrderType.STOP_LIMIT.upper()}" for item in TradeType.list_values()]:
