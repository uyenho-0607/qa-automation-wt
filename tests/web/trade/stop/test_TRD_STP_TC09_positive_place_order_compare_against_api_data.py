...
def test(web, stop_obj, get_asset_tab_amount, symbol, close_confirm_modal, update_trade_obj):
    step_idx = 2

    logger.info(f"Step 1: Get details of current placed {OrderType.STOP.upper()!r} order")
    trade_object = update_trade_obj(web.trade_page.asset_tab.get_item_data(AssetTabs.PENDING_ORDER))
    ...
    logger.info(f"Step {step_idx}: Get placed order API data (ID:{trade_object.order_id!r})")
...
def update_trade_obj(symbol):
    def _handler(order_data):
        ...
        if order_data.get("order_type") in [f"{item.upper()} {OrderType.STOP.upper()}" for item in TradeType.list_values()]:
