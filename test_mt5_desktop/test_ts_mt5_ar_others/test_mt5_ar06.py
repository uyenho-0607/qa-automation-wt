...
from common.desktop.module_trade.utils import toggle_radio_button, button_trade_module, dropdown_order_type, button_buy_sell_type, verify_volume_min_max_buttons, btn_min_max_price, btn_min_max_stop_limit_price, btn_min_max_stop_loss, btn_min_max_take_profit
...
class TC_aR06():
    ...
    @allure.title("TC_aR06")
    
    @allure.description(
        """
        (Place) - Pending Order (Stop Limit)
    ...
    def test_tc06(self, chrome_driver, request):
        ...
        try:
            ...
            with allure.step("Select the orderType option: Stop Limit"):
                dropdown_order_type(driver=main_driver, partial_text="stop-limit")
            ...
            with allure.step("Increase / Decrease Entry Price"):
                btn_min_max_price(driver=main_driver, trade_type="trade", min_max="increase", number_of_clicks=5)
                btn_min_max_price(driver=main_driver, trade_type="trade", min_max="decrease", number_of_clicks=3)

            with allure.step("Increase / Decrease Stop Limit Price"):
                btn_min_max_stop_limit_price(driver=main_driver, trade_type="trade", min_max="decrease", number_of_clicks=5)
                btn_min_max_stop_limit_price(driver=main_driver, trade_type="trade", min_max="increase", number_of_clicks=3)
