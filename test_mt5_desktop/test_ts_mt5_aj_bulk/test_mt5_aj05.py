...
from enums.main import Server, OrderPanel, BulkActionType, SectionName, CSVFileNameManager
...
from data_config.api.request import create_mt5_pending_order
    ...
    def test_tc05(self, chrome_driver, request):
        ...
        try:
            ...
            with allure.step("Search symbol"):
                input_symbol(driver=main_driver, server=Server.MT5, desired_symbol_name="XAUUSD")

            with allure.step("Create bulk market orders"):
                create_mt5_pending_order(driver=main_driver)
                
            with allure.step("Bulk Delete Orders"):
                clear_order_ids_csv(filename=CSVFileNameManager.MT5_DESKTOP_BULK)
                pending_order_df = button_bulk_operation(driver=main_driver, filename=CSVFileNameManager.MT5_DESKTOP_BULK, bulk_type=BulkActionType.BULK_DELETE, section_name=SectionName.TRADE_PENDING_ORDER, tab_order_type=OrderPanel.PENDING_ORDERS, set_sorting=True)
            ...
            with allure.step("Ensure the OrderID is display in order panel: Order History table"):
                ...
                order_history_df = check_order_ids_in_table(driver=main_driver, order_ids=csv_order_ids, tab_order_type=OrderPanel.ORDER_AND_DEALS, section_name=SectionName.ORDER_HISTORY)

            with allure.step("Comparison on Order History and Pending Order table"):
                compare_dataframes(driver=main_driver, df1=order_history_df, name1=SectionName.ORDER_HISTORY, df2=pending_order_df, name2=SectionName.TRADE_PENDING_ORDER)
