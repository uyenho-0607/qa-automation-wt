...
from common.desktop.module_trade.utils import (
    toggle_radioButton, trade_market_order, modify_market_order, 
    trade_ordersConfirmationDetails, get_trade_snackbar_banner, extract_order_info
)
...
class TC_MT5_aB01():
    ...
    def test_tc01(self, chromeDriver, request):
        ...
        test_failed = False  # Track if the test fails
        ...
        try:
            ...
            with allure.step("Place Market Order"):
                trade_market_order(driver=main_driver, trade_type="trade", option="buy", set_fillPolicy=True, set_stopLoss=False, set_takeProfit=False)
            ...
            with allure.step("Retrieve the Newly Created Open Position Order"):
                original_orderID, trade_order_df = extract_order_info(driver=main_driver, tab_order_type="open-positions",
                                                                      section_name="Trade Open Position", row_number=[1])
            ...
            with allure.step("Retrieve the Order Panel data"):
                updated_orderID, updated_order_df = extract_order_info(driver=main_driver, tab_order_type="open-positions",
                                                                       section_name="Updated Open Position", row_number=[1])
            ...
            with allure.step("Verify if it is the same orderIDs"):
                assert original_orderID == updated_orderID, f"Mismatch: {original_orderID} vs {updated_orderID}"
            ...
            # Attach profiling results to Allure report
            attach_text(get_text=profiler_output.getvalue(), name="Performance Profiling Results")

            attach_session_video_to_allure(session_id)

            rerun_marker = request.node.get_closest_marker("flaky")
            if rerun_marker:
                reruns = rerun_marker.kwargs.get("reruns", 0)
                current_attempt = getattr(request.node, "execution_count", 1)
                last_attempt = current_attempt >= (reruns + 1)
            else:
                last_attempt = True
