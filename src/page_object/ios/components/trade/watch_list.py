...
class WatchList(BaseTrade):
    ...
    # ------------------------ LOCATORS ------------------------ #
    __tab = (AppiumBy.ACCESSIBILITY_ID, "tab-{}")
    __sub_tab = (AppiumBy.ACCESSIBILITY_ID, "{}")
    __items = (AppiumBy.ACCESSIBILITY_ID, "watchlist-list-item")
    __item_by_name = (
        AppiumBy.IOS_CLASS_CHAIN, "**/XCUIElementTypeOther[`name == 'watchlist-list-item' AND label CONTAINS '{}'`]"
    )
    __star_icon_by_symbol = (AppiumBy.ACCESSIBILITY_ID, "chart-star-symbol")
    __btn_symbol_remove = (AppiumBy.IOS_CLASS_CHAIN, "**/XCUIElementTypeStaticText[`name == 'REMOVE'`]")

    # ------------------------ ACTIONS ------------------------

    def select_tab(self, tab: WatchListTab, timeout=SHORT_WAIT):
        """Handle selecting tab for home screen"""
        logger.info(f"- Select tab: {tab.value.capitalize()!r}")
        locator = cook_element(self.__tab, tab.get_tab())

        if self.actions.is_element_displayed(locator, timeout):
            self.actions.click(locator)
            return

        logger.debug(f"- Tab is sub-tab, select {WatchListTab.ALL.value!r} first")
    ...
    def get_current_symbols(self, tab: WatchListTab = None, random_symbol=False):
        ...
        # Extract first word that starts with an alphanumeric character
        extracted_symbols = [
    ...
    def verify_symbols_displayed(self, tab: WatchListTab, symbols: str | list = None, is_display=True,
                                 timeout=SHORT_WAIT):
        """Verify symbol is displayed in tab"""
        self.select_tab(tab)
        symbols = symbols if isinstance(symbols, list) else [symbols]
        for symbol in symbols:
            self.actions.verify_element_displayed(cook_element(self.__item_by_name, symbol), is_display=is_display,
                                                  timeout=timeout)
