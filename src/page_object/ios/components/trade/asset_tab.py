...
class AssetTab(BaseTrade):
    ...
    # ------------------------ LOCATORS ------------------------ #
    __tab = (AppiumBy.ACCESSIBILITY_ID, "tab-asset-order-type-{}")
    __tab_amount = (AppiumBy.XPATH, "//*[@name='tab-asset-order-type-{}' and contains(@label, '({})')]")

    # Item details locators
    __item_order_no = (AppiumBy.ACCESSIBILITY_ID, 'asset-{}-list-item-order-no')
    __item_by_order_no = (
        AppiumBy.XPATH,
        "//*[@resource-id='asset-{}-list-item-order-no' and contains(@text(), '{}')]/parent::android.view.ViewGroup"
    )
    __expand_item = (AppiumBy.XPATH, "asset-{}-list-item-expand")
