...
class AssetTab(BaseTrade):
    ...
    __item_symbol = (AppiumBy.ID, 'asset-detailed-header-symbol')
    __item_order_type = (AppiumBy.ID, 'asset-order-type')
    ...
    def get_last_order_id(self, trade_object: ObjTrade = None, tab: AssetTabs = None, wait=True) -> int:
        """
    ...
    def _click_action_btn(self, tab: AssetTabs, order_id=0, action: Literal["edit", "close"] = "edit"):
    ...
    def delete_last_order(self, trade_object: ObjTrade = None, confirm=True, wait=True):
        order_id = self.get_last_order_id(tab=AssetTabs.PENDING_ORDER, wait=wait)

        # update orderID for trade object
        if trade_object:
            trade_object.order_id = order_id

        logger.debug(f"- Delete latest order with ID: {order_id!r}")
        self._click_action_btn(AssetTabs.PENDING_ORDER, order_id, "close")  # close == delete in pending orders tab

        if confirm:
            self.__trade_modals.confirm_delete_order()

    def delete_order(self, order_id, confirm=True):
        logger.debug(f"- Delete order with ID: {order_id!r}")
        self._click_action_btn(AssetTabs.PENDING_ORDER, order_id, "close")  # close == delete in pending orders tab
        if confirm:
            self.__trade_modals.confirm_delete_order()
    ...
    def modify_order(
        ...
        """
        # click action button
        self._click_action_btn(AssetTabs.get_tab(trade_object.order_type), trade_object.get("order_id"), "edit")
