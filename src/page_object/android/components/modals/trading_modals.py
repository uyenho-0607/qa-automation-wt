...
class TradingModals(BaseTrade):
    ...
    __confirm_order_type = (AppiumBy.ID, 'trade-confirmation-order-type')
    __confirm_symbol = (AppiumBy.ID, 'trade-confirmation-symbol')
    __confirm_labels = (AppiumBy.ID, 'trade-confirmation-label')
    __confirm_values = (AppiumBy.ID, 'trade-confirmation-value')

    ##### Edit Trade Modal #####
    __edit_symbol_price = (AppiumBy.ID, 'edit-symbol-price')
    __btn_edit_order = (AppiumBy.ID, 'edit-button-order')
    __btn_cancel_edit_order = (AppiumBy.ID, 'edit-button-cancel')
    __txt_edit_sl = (AppiumBy.ID, 'edit-input-stoploss-{}')  # price or points
    __txt_edit_tp = (AppiumBy.ID, 'edit-input-takeprofit-{}')  # price or points
    __txt_edit_price = (AppiumBy.ID, 'edit-input-price')
    __txt_edit_stop_limit_price = (AppiumBy.ID, 'edit-input-stop-limit-price')
    __drp_expiry = (AppiumBy.ID, 'edit-dropdown-expiry')
    __opt_edit_expiry = (AppiumBy.ID, 'edit-dropdown-expiry-{}')
    __expiry_date = (AppiumBy.ID, 'edit-input-expiry-date')
    ...
    __edit_confirm_labels = (AppiumBy.ID, 'edit-confirmation-label')
    __edit_confirm_values = (AppiumBy.ID, 'edit-confirmation-value')
    __edit_confirm_order_id = (AppiumBy.ID, 'edit-confirmation-order-id')
    __edit_confirm_order_type = (AppiumBy.ID, 'edit-confirmation-order-type')
    __edit_confirm_symbol = (AppiumBy.ID, 'edit-confirmation-symbol')
    __btn_confirm_update_order = (AppiumBy.ID, 'edit-confirmation-button-confirm')

    ##### Asset Items #####
    __btn_cancel_close_order = (AppiumBy.ID, 'close-order-button-cancel')
    __btn_cancel_delete_order = (AppiumBy.ID, 'confirmation-modal-button-cancel')

    __btn_bulk_close_confirm = (AppiumBy.ID, 'bulk-close-modal-button-submit-{}')
    __btn_bulk_close_cancel = (AppiumBy.ID, 'bulk-close-modal-button-cancel-all')
    ...
    def confirm_update_order(self, confirm=True, timeout=EXPLICIT_WAIT):
        ...
        self.actions.click(self.__btn_confirm_update_order if confirm else self.__btn_cancel_edit_order, timeout=timeout, raise_exception=not confirm, show_log=not confirm)
