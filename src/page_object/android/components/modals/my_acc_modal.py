...
class MyAccountModal(BaseScreen):
    ...
    def is_open(self):
        return self.actions.is_element_displayed(cook_element(self.__items, AccSummary.STOP_OUT_LEVEL), timeout=SHORT_WAIT)
    ...
    def toggle_balance(self, open=True):
        is_open = self.actions.is_element_displayed(cook_element(self.__items, AccSummary.BALANCE), timeout=SHORT_WAIT)
        if is_open != open:
            self.actions.click(self.__drp_balance)

    def toggle_note(self, open=True):
        is_open = self.actions.is_element_displayed(cook_element(self.__items, AccSummary.STOP_OUT_LEVEL), timeout=SHORT_WAIT)
        if is_open != open:
    ...
    def verify_account_info(self, exp_dict):
        ...
        soft_assert(actual, exp_dict, tolerance=5, tolerance_fields=AccSummary.list_values(except_val=AccSummary.BALANCE))
