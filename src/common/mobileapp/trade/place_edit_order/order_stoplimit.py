...
from common.mobileweb.trade.orderPanel_info import button_orderPanel_action
from common.mobileweb.trade.orderPlacingWindow import button_trade_action, dropdown_orderType, expiry, handle_entryPrice, handle_stopLimitPrice, handle_stopLoss, handle_takeProfit, input_size_volume, label_onePointEqual
from common.mobileweb.trade.chart import minMax_Chart
from common.mobileweb.trade.place_edit_order.price_related import get_current_price, get_edit_order_label, get_random_point_distance, get_sl_point_distance, get_tp_point_distance, pointsDistance
...
def calculate_stopLimit_stopLoss(driver, trade_type, sl_type, option, label_onePointsEqual):
    ...
    if sl_type == "price":
        ...
        if option in ["buy", "BUY STOP LIMIT"]:
            # stopLoss_value = (Stop Limit Price - Price field) / One point equal
            # stopLoss_value = int((stopLimitPrice_value - low_price)/label_onePointsEqual)
            stopLoss_value = stopLoss_point

        elif option in ["sell", "SELL STOP LIMIT"]:
            # stopLoss_value = (Price field - Stop Limit Price) / One point equal
            # stopLoss_value = int((high_price - stopLimitPrice_value) / label_onePointsEqual)
            stopLoss_value = stopLoss_point
...
def calculate_stopLimit_takeProfit(driver, trade_type, tp_type, option, label_onePointsEqual):
    ...
    if tp_type == "price":
        ...
        if option in ["buy", "BUY STOP LIMIT"]:
            # takeProfit_value = (Price field - Stop Limit Price) / One point equal
            # takeProfit_value = int((high_price - stopLimitPrice_value) / label_onePointsEqual)
            takeProfit_value = takeProfit_point

        elif option in ["sell", "SELL STOP LIMIT"]:
            # takeProfit_value = (Stop Limit Price - Price field) / One point equal
            # takeProfit_value = int((stopLimitPrice_value - low_price) / label_onePointsEqual)
            takeProfit_value = takeProfit_point
