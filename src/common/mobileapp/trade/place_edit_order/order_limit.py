...
from common.mobileweb.trade.orderPlacingWindow import label_onePointEqual, button_trade_action, expiry, handle_entryPrice, handle_stopLoss, handle_takeProfit, input_size_volume
...
def calculate_limit_stopLoss(driver, trade_type, sl_type, option, label_onePointsEqual):
    ...
    if sl_type == "price":
        ...
        if option in ["buy", "BUY LIMIT"]:
            # stopLoss_value = (Price(L) - Price field) / One point equal
            # stopLoss_value = int((entryPrice_value - low_price)/label_onePointsEqual)
            stopLoss_value = stopLoss_point

        elif option in ["sell", "SELL LIMIT"]:
            # stopLoss_value = (Price field - Price(L)) / One point equal
...
def calculate_limit_takeProfit(driver, trade_type, tp_type, option, label_onePointsEqual):
    ...
    if tp_type == "price":
        ...
        if option in ["buy", "BUY LIMIT"]:
            # takeProfit_value = (Price field - Price(L)) / One point equal
            # takeProfit_value = int((high_price - entryPrice_value) / label_onePointsEqual)
            takeProfit_value = takeProfit_point

        elif option in ["sell", "SELL LIMIT"]:
            # takeProfit_value = (Price(L) - Price field) / One point equal
...
def trade_limit_order(driver, trade_type, option, expiryType, expiryDate=None, targetMonth=None, hour_option=None, min_option=None, sl_type=None, tp_type=None, chart_fullscreen=None, set_Chart: bool = False, set_entryPrice: bool = True, set_stopLoss: bool = True, set_takeProfit: bool = True, specifiedDate: bool = False):
    try:
