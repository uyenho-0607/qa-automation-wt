...
def verify_bulk_button_disabled(driver, tab_order_type: OrderPanel, bulk_test_id):
...
def verify_read_only_access(driver, tab_order_type: OrderPanel, bulk_test_id):
    
    """Verify read-only access restrictions by checking UI elements and bulk actions."""
    menu_button(driver, menu=Menu.ASSETS)  # Navigate to "Assets"
...
def read_only_access(driver, set_menu: bool = False):
    ...
    try:
        ...
        tab_actions = {
            OrderPanel.OPEN_POSITIONS: "bulk-close",
            OrderPanel.PENDING_ORDERS: "bulk-delete"
        }
