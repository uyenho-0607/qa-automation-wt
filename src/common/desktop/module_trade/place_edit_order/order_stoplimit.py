...
from common.desktop.module_trade.place_edit_order.price_related import  get_current_price, get_edit_order_label, calculate_pending_entry_price, calculate_stop_limit_price, calculate_pending_stop_loss, calculate_pending_take_profit
...
def trade_stop_limit_order(driver,  option: TradeDirectionOption, expiry_type: ExpiryType, trade_type: ButtonModuleType = ButtonModuleType.TRADE, 
                          ...
                          stop_limit_price_flag: AlertType = AlertType.POSITIVE, stop_loss_flag: AlertType = AlertType.POSITIVE, take_profit_flag: AlertType = AlertType.POSITIVE):
    try:
        ...
        calculate_stop_limit_price(driver, trade_type, option, label_one_points_equal, stop_limit_price_flag)

        # Set Stop Loss if specified or required        
        if TradeConstants.SET_STOP_LOSS in trade_constants or sl_type:
            calculate_pending_stop_loss(driver, trade_type, sl_type, option, label_one_points_equal, stop_loss_flag, is_stop_limit=True)

        # Set Take Profit if specified or required
        if TradeConstants.SET_TAKE_PROFIT in trade_constants or tp_type:
            calculate_pending_take_profit(driver, trade_type, tp_type, option, label_one_points_equal, take_profit_flag, is_stop_limit=True)
