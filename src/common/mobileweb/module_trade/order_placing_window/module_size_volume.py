...
from constants.helper.element import click_element, click_element_with_wait, clear_input_field, find_element_by_testid, visibility_of_element_by_testid, populate_element_with_wait
...
def input_size_volume(driver, desired_state="units", swap: bool = True):
    try:
        delay(0.5)
...
def btn_minMax_size(driver, minMax, number_of_clicks):
    try:
        ...
        button_minMax = visibility_of_element_by_testid(driver, data_testid=f"trade-input-volume-{minMax}")
        
        # Find the input field for the trade volume
        input_field = visibility_of_element_by_testid(driver, data_testid=f"trade-input-volume")
