...
def wt_user_login(driver, platform: str, client_name: str, device_type: str, env_type: str, testcaseID: str = None, expect_failure: bool = False, use_crm_cred: bool = False) -> None:
    ...
    if platform in data:
        # Use the sub_key (e.g., "Lirunex") to get the specific platform data
...
def handle_login_error(driver):
    ...
    error_message_notification = visibility_of_element_by_testid(driver, data_testid="notification-box-description")
    error_message = get_label_of_element(error_message_notification)
    attach_text(error_message, name="Error message found:")
    find_element_by_testid(driver, data_testid="notification-box-close")
...
def handle_login_result(driver, expect_failure=False):
    ...
    match = wait_for_text_to_be_present_in_element_by_xpath(driver, "//div[normalize-space(text())='Available Balance']", text="Available Balance")
    ...
    if match:
        ...
        # Verify successful redirection and attach confirmation
        # visibility_of_element_by_testid(driver, data_testid="symbol-search-selector")
...
# Login to WebTrader Website
def login_wt(driver, account_type, platform: str, client_name: str, testcaseID: str = None, device_type: str = "Mobile", env_type: str = "Release_SIT", expect_failure: bool = False, use_crm_cred: bool = False) -> None:
    try:
